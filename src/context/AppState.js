...
const AppState = (props) => {
    ...
    const [sortFilter, setSortFilter] = useState({filter:false,filterStyle: '', sort:false, type: '', })
    const [page, setPage] = useState({landingPage:false, categoryItems: false })
 ...
 /*     async function getData(){
    ...
    function addBlankGrids(array,width,height){
     ...
     const blankGridArr = Array(blankGridsNum).fill(0);
      return (
          blankGridArr.map((item,index)=>{
              return <Grid item style={{width:width, height:height, opacity:0}}></Grid>
          })
      )
  }

   
    const initialState = {  //intial state object has to come before the usereducer or I will keep getting undefined 
    ...
    function reducer(currentState, action){
     //console.log('I am the payload:' , action.payload)
      switch (action.type) {
        ...
        const config = CreateFilter(currentState.massages, 'title', action.payload);
